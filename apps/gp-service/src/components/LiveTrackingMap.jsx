...
export default function LiveTrackingMap({ tracking, className = 'h-72', showRoute = true }) {
  ...
  useEffect(() => {
    ...
    geofences.forEach((z) => {
      ...
      circle.bindPopup(`<b>${z.name}</b><br/>Официальный слив`)
      layersRef.current[`zone-${z.id}`] = circle
    })

    if (client?.lat != null) {
      const m = L.marker([client.lat, client.lng], { icon: dot(GEOFENCE_COLORS.client, 16) }).addTo(map)
      m.bindPopup('Клиент')
      layersRef.current.client = m
      L.circle([client.lat, client.lng], {
        radius: 80,
        color: GEOFENCE_COLORS.client,
        fillOpacity: 0.08,
        weight: 1,
        dashArray: '4 4',
      }).addTo(map)
    ...
    if (executor?.lat != null) {
      const m = L.marker([executor.lat, executor.lng], { icon: dot(GEOFENCE_COLORS.executor, 16) }).addTo(map)
      m.bindPopup(`Исполнитель · ${tracking.statusLabel || ''}`)
      layersRef.current.executor = m
    }

    if (showRoute && route.length > 1) {
      const line = L.polyline(route, { color: GEOFENCE_COLORS.executor, weight: 4, opacity: 0.7 }).addTo(map)
