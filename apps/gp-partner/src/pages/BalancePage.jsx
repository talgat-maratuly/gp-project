...
export default function BalancePage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-[var(--gp-text)] mb-4">Баланс</h1>
      ...
      <div className="partner-card p-4 mb-4 text-sm text-[var(--gp-text-muted)] space-y-2">
        <p className="font-semibold text-[var(--gp-text)]">Комиссия GP</p>
        ...
        <p className="text-[10px] text-[var(--gp-text-muted)] pt-1">Оплата клиента идёт вам напрямую (Kaspi / наличные). GP не держит деньги клиента.</p>
      </div>

      <p className="text-sm font-semibold text-[var(--gp-text)] mb-2">Пополнить баланс</p>
      <div className="grid grid-cols-2 gap-2 mb-6">
        {TOPUPS.map((a) => (
          <button key={a} type="button" disabled={loading} onClick={() => topup(a)} className="py-3 rounded-xl bg-[var(--gp-surface-2)] border border-[var(--gp-border)] text-sm font-semibold hover:border-emerald-500/40 disabled:opacity-50">
            +{formatPrice(a)}
          </button>
        ))}
      </div>

      <p className="text-sm font-semibold text-[var(--gp-text)] mb-2">История</p>
        ...
        {transactions.map((t) => (
          <li key={t.id} className="partner-card p-3 flex items-center gap-3 text-sm">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[var(--gp-text)] truncate">{t.note}</p>
              <p className="text-[10px] text-[var(--gp-text-muted)]">{new Date(t.createdAt).toLocaleString('ru-KZ')}</p>
            </div>
            <span className={`font-bold shrink-0 ${t.amount > 0 ? 'text-emerald-500' : 'text-red-500'}`}>
              {t.amount > 0 ? '+' : ''}{formatPrice(Math.abs(t.amount))}
            </span>
          </li>
        ))}
        {!transactions.length && <p className="text-[var(--gp-text-muted)] text-sm">Пока нет операций</p>}
      </ul>
    </div>
  )
}
