...
/** Услуга: templateId или id вида lawn_fr-uralsk → service_lawn */
export function useServiceLabel() {
  const { t } = useLanguage()
  return (serviceId) => {
    if (!serviceId) return ''
    const tpl = String(serviceId).replace(/_fr-[a-z0-9-]+$/i, '')
    return t(`service_${tpl}`) !== `service_${tpl}` ? t(`service_${tpl}`) : serviceId
  }
}
